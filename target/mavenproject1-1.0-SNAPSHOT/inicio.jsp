<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    <title>Banco FLAD - Inicio</title>
    <link rel="stylesheet" type="text/css" href="style.css">
</head>
<body>
    <header>
        <h1>Bienvenidos a Banco FLAD</h1>
        <p>Comprometidos con tu futuro financiero.</p>
    </header>

    <section>
        <h2>Accede a tu cuenta o crea una nueva</h2>
        <button onclick="window.location.href='login.jsp';">Iniciar Sesión</button>
        <button onclick="window.location.href='index.jsp';">Crear Nuevo Usuario</button>
    </section>

    <article>
        <h2>Sobre el Banco</h2>
        <p>Banco FLAD, fundado en 2024, es uno de los bancos más confiables y con mayor crecimiento en la región. Ofrecemos una amplia gama de servicios financieros a más de un millón de clientes a lo largo del país.</p>
    </article>

    <footer>
        <h2>Nuestro Equipo Gerencial</h2>
        <div class="gerente">
            <img src="images/gerente1.jpg" alt="Gerente 1">
            <p>Andrés David Pérez Cely - CEO </p>
            <p>Contacto: adperezcely@gmail.com</p>
        </div>
        <div class="gerente">
            <img src="images/gerente2.jpg" alt="Gerente 2">
            <p>Daniel Gonzalez - CFO</p>
            <p>Contacto: danielfernandog14@gmail.com</p>
        </div>
        <div class="gerente">
            <img src="images/gerente2.jpg" alt="Gerente 2">
            <p>Juan Felipe Galvis - CFO</p>
            <p>Contacto: correoDeJuan@gmail.com</p>
        </div>
        <div class="gerente">
            <img src="images/gerente2.jpg" alt="Gerente 2">
            <p>Luis Felipe Gutierrez - CFO</p>
            <p>Contacto: correoDeLuis@gmail.com</p>
        </div>
    </footer>
</body>
</html>
